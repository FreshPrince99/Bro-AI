...
// .wrangler/tmp/bundle-LCb0At/strip-cf-connecting-ip-header.js
...
// src/durable-objects/userCoachDO.ts
var UserCoachDO = class {
  state;
  constructor(state) {
    this.state = state;
  }
  async fetch(request) {
    const url = new URL(request.url);
    const storedState = await this.state.storage.get("state");
    const state = storedState ?? { profile: void 0, conversation: [], plan: void 0, calendarEventIds: [] };
...
__name(UserCoachDO, "UserCoachDO");
...
app.post("/api/chat", async (c) => {
  ...
  const aiResponse = {
    role: "assistant",
    content: "This is a placeholder response. Connect Workers AI by calling env.AI.run(...) with your prompt and user context."
  };
...
var scheduled = /* @__PURE__ */ __name(async (_event, env2, ctx) => {
  const remindRequest = new Request("https://worker/api/remind", { method: "POST" });
  ctx.waitUntil(app.fetch(remindRequest, env2, ctx));
}, "scheduled");
var src_default = { fetch: app.fetch, scheduled };
...
// .wrangler/tmp/bundle-LCb0At/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LCb0At/middleware-loader.entry.ts
